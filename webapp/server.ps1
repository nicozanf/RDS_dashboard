...
$script:ApplicationVersion = '2026.09.08'
...
function Initialize-FarmMetricsStore {
    $localSqliteExe = Join-Path $scriptRoot 'sqlite3.exe'
    ...
    if ([string]::IsNullOrWhiteSpace([string]$script:sqliteExe)) {
        Write-Log "sqlite3.exe not found. 24h farm history persistence is disabled."
        $script:sqliteEnabled = $false
        return
    }
...
"@
    ...
    try {
        $initOut = (& $script:sqliteExe $farmMetricsDbPath $createSql 2>&1 | Out-String).Trim()
        if ($LASTEXITCODE -ne 0) {
            throw $initOut
        }
        ...
        $schemaRows = @(& $script:sqliteExe -noheader -separator '|' $farmMetricsDbPath $schemaSql 2>&1)
        if ($LASTEXITCODE -ne 0) {
        ...
        if (-not $hasSessionsColumn) {
            ...
            $migrateOut = (& $script:sqliteExe $farmMetricsDbPath $migrateSql 2>&1 | Out-String).Trim()
            if ($LASTEXITCODE -ne 0) {
                throw $migrateOut
            }
        }
        ...
        Write-Log "SQLite 24h farm metrics store initialized at $farmMetricsDbPath"
    }
    catch {
        $script:sqliteEnabled = $false
        Write-Log "Failed to initialize SQLite store: $($_.Exception.Message)"
    }
}
...
function Get-DataPayload {
    ...
    try {
        ...
        Write-Log "Failed to enrich payload with server history: $($_.Exception.Message)"
        return $payload
    }
}
...
function Invoke-SessionCommand {
    ...
    try {
        ...
        Write-AuditLog -Event $Action -Actor $MessageSender -Server $Server -SessionId $Id -Username $Username -SessionName $SessionName -State '' -Details 'Action completed successfully'
...
function Get-DashboardHtml {
    ...
    $domainShort = 'MYDOMAIN'
    $searchDomain = if (-not [string]::IsNullOrWhiteSpace($customLoginDomain)) { $customLoginDomain } else { $DefaultDomain }
    if (-not [string]::IsNullOrWhiteSpace($searchDomain)) {
        $domainRoot = ([string]$searchDomain).Trim().Split('.', 2)[0]
        if (-not [string]::IsNullOrWhiteSpace($domainRoot)) {
            $domainShort = $domainRoot.ToUpperInvariant()
        }
    }
    $adSearchLabel = [System.Web.HttpUtility]::HtmlEncode(('Search users in the {0} AD' -f $domainShort))

    $template = Get-HtmlTemplate -TemplateName 'dashboard.html'
    return $template.Replace('__DASHBOARD_TITLE__', $dashboardTitle).Replace('__SAFE_USER__', $safeUser).Replace('__CSRF_TOKEN__', $safeCsrfToken).Replace('__AD_SEARCH_LABEL__', $adSearchLabel).Replace('__FLASH_MESSAGE__', $safeFlashMessage).Replace('__AUTO_REFRESH_TEXT__', $autoRefreshText)
}
...
function Save-CurrentConfigState {
    ...
    Write-ConfigFile -Path $ConfigFile -Title ([string]$State.Title) -Farms $State.Farms -CollectorTimeoutSeconds ([int]$State.CollectorTimeoutSeconds) -CycleDelaySeconds ([int]$State.CycleDelaySeconds)
...
function Get-CollectorProcess {
    ...
    try {
        if ($collectorJob -is [System.Diagnostics.Process]) {
            return $collectorJob
        }

        if ($collectorJob.PSObject.Properties['Id']) {
            return (Get-Process -Id ([int]$collectorJob.Id) -ErrorAction SilentlyContinue)
        }
    }
    catch {}
...
function Get-CollectorJobState {
    $collectorProcess = Get-CollectorProcess
    ...
    try {
        if ($collectorProcess.HasExited) {
            $collectorPid = [int]$collectorProcess.Id
            $exitCode = $null
            try {
                $exitCode = [int]$collectorProcess.ExitCode
            }
            catch {}

            if ($script:collectorLastObservedPid -ne $collectorPid -or $script:collectorLastObservedState -ne 'Stopped' -or $script:collectorLastObservedExitCode -ne $exitCode) {
                Write-Log "Collector process exited (pid=$collectorPid, exitCode=$exitCode). stdout=$collectorStdOutLog stderr=$collectorStdErrLog"
                $script:collectorLastObservedPid = $collectorPid
                $script:collectorLastObservedState = 'Stopped'
                $script:collectorLastObservedExitCode = $exitCode
            }
            return 'Stopped'
        }
    }
    ...
    $runningPid = [int]$collectorProcess.Id
    if ($script:collectorLastObservedPid -ne $runningPid -or $script:collectorLastObservedState -ne 'Running') {
        Write-Log "Collector process is running (pid=$runningPid). stdout=$collectorStdOutLog stderr=$collectorStdErrLog"
        $script:collectorLastObservedPid = $runningPid
        $script:collectorLastObservedState = 'Running'
        $script:collectorLastObservedExitCode = $null
    }
...
function Stop-CollectorJob {
    ...
    if ($null -ne $collectorProcess) {
        try {
            if (-not $collectorProcess.HasExited) {
                Write-Log "Stopping collector process pid=$($collectorProcess.Id)"
    ...
    $script:collectorLastObservedPid = 0
    $script:collectorLastObservedState = 'NotStarted'
    $script:collectorLastObservedExitCode = $null
}
...
function Start-CollectorJob {
    ...
    try {
        ...
        # Keep original value; collector has fallback path resolution.
    }

    $hostExecutable = Resolve-CollectorHostExecutable
    $collectorArgs = @(
        '-NoLogo',
        '-NoProfile',
        '-NonInteractive',
        ...
        ([string]$CollectorTimeoutSeconds),
        '-CycleDelaySeconds',
        ([string]$CycleDelaySeconds),
        '-ConfigFile',
        $collectorConfigPath
    )

    try {
        if (Test-Path -LiteralPath $collectorStdOutLog) {
            Remove-Item -LiteralPath $collectorStdOutLog -Force -ErrorAction SilentlyContinue
        }
        if (Test-Path -LiteralPath $collectorStdErrLog) {
            Remove-Item -LiteralPath $collectorStdErrLog -Force -ErrorAction SilentlyContinue
        }
    }
    catch {}

    $argLine = ($collectorArgs | ForEach-Object {
            if ([string]::IsNullOrWhiteSpace([string]$_)) { '""' }
            elseif ([string]$_ -match '\s') { '"' + ([string]$_ -replace '"', '""') + '"' }
            else { [string]$_ }
        }) -join ' '

    Write-Log "Starting collector process using host '$hostExecutable'"
    Write-Log "Collector launch arguments: $argLine"
    Write-Log "Collector output redirection: stdout=$collectorStdOutLog stderr=$collectorStdErrLog"

    $proc = Start-Process -FilePath $hostExecutable -ArgumentList $argLine -WorkingDirectory $scriptRoot -RedirectStandardOutput $collectorStdOutLog -RedirectStandardError $collectorStdErrLog -PassThru

    Write-Log "Collector process started with pid=$($proc.Id)"
    $script:collectorLastObservedPid = [int]$proc.Id
    $script:collectorLastObservedState = 'Running'
    $script:collectorLastObservedExitCode = $null
    return $proc
}
...
function Get-CollectorLogTail {
    ...
    try {
        $lines = @(Get-Content -LiteralPath $Path -Tail ([math]::Max(1, $LineCount)) -ErrorAction Stop)
        if ($lines.Count -eq 0) {
            return ''
        }
...
function Get-CollectorRestartMetrics {
    ...
    if (($lastOutcome -eq 'Failed' -or $lastOutcome -eq 'CooldownBlocked') -and $cooldownRemainingSeconds -gt 0) {
        $warningActive = $true
        $warningReason = if ($lastOutcome -eq 'Failed') { 'Recent collector restart failure' } else { 'Collector restart cooldown active after repeated restart requests' }
    }
    elseif ($requestsLastHour -ge 6) {
        $warningActive = $true
        $warningReason = 'High collector restart rate in the last hour'
    }
        ...
        if (($now - $lastCollectorCheck).TotalSeconds -ge $collectorCheckIntervalSeconds) {
            ...
            if ($null -ne $collectorJob) {
                ...
                if ($jobState -ne 'Running') {
                    ...
                    $snapshotMeta = Get-SnapshotMetadata
                    $forceRestart = ($jobState -eq 'Stopped' -and $snapshotMeta.IsStale)
                    $restartResult = Restart-CollectorJob -Actor 'system' -IgnoreCooldown:$forceRestart
                    if ($restartResult.Success) {
