...
$script:sqliteWriteAttempts = 0
$script:sqliteWriteRetryCount = 0
$script:sqliteWriteLockErrors = 0
$script:perfLogEveryCycles = 20
...
function Inizia {
    ...
    if ([string]::IsNullOrWhiteSpace($appDir)) {
        $collectorLogsDir = Join-Path $RuntimeDir 'logs'
    }
    else {
        $collectorLogsDir = Join-Path $appDir 'logs'
    }
    if (-not (Test-Path -LiteralPath $collectorLogsDir)) {
        New-Item -ItemType Directory -Path $collectorLogsDir -Force | Out-Null
    }
    $script:collectorLogPath = Join-Path $collectorLogsDir 'collector.log'
    ...
    $script:farmMetricsDbPath = Join-Path $RuntimeDir 'farm_metrics.sqlite'
...
function Build-CyclePayload {
    ...
    $maxConcurrentJobs = [math]::Min($serversCount, 12)
    ...
    $jobScript = {
        param($srv, $cimTimeoutSeconds, $quserTimeoutSeconds, $collectorTimeoutSeconds)
    ...
    $jobPoolResult = Invoke-JobPool -MaxConcurrentJobs $maxConcurrentJobs -TimeoutSeconds $CollectorTimeoutSeconds -JobScript $jobScript -InputObjects @($script:servers) -JobPrefix 'RdsCollector' -ArgumentList @($cimTimeoutSeconds, $quserTimeoutSeconds, $CollectorTimeoutSeconds)
...
function Write-CollectorLog {
    ...
    Write-Host $line
    try {
        $logParent = Split-Path -Path $script:collectorLogPath -Parent
        if (-not [string]::IsNullOrWhiteSpace($logParent) -and -not (Test-Path -LiteralPath $logParent)) {
            New-Item -ItemType Directory -Path $logParent -Force | Out-Null
        }
        Add-Content -LiteralPath $script:collectorLogPath -Value $line -Encoding UTF8
    } catch {
...
function Initialize-MetricsStore {
    ...
    if ([string]::IsNullOrWhiteSpace([string]$script:sqliteExe)) {
        throw 'sqlite3.exe not found. SQLite metric persistence is mandatory for collector startup.'
    }
...
function Main {
    ...
    while ($true) {
        try {
            ...
            try {
                Set-MetricsFromPayload -Payload $payload
            } catch {
                Write-CollectorLog "ERROR in Set-MetricsFromPayload: $($_.Exception.Message)"
            }

            if (($cycleCounter % [math]::Max(1, [int]$script:perfLogEveryCycles)) -eq 0) {
                try {
                    $cycleMetrics = Get-CycleMetrics
                    $retryRate = if ($script:sqliteWriteAttempts -gt 0) {
                        [math]::Round((100.0 * $script:sqliteWriteRetryCount / $script:sqliteWriteAttempts), 1)
                    }
                    else {
                        0.0
                    }
                    ...
                    Write-CollectorLog (
                        "PERF: cycles={0} avg={1}s p50={2}s p95={3}s min={4}s max={5}s window={6} sqliteWrites={7} sqliteRetries={8} retryRate={9}% sqliteLockErrors={10}" -f
                        $cycleMetrics.CycleCount,
                        $cycleMetrics.AvgSeconds,
                        $cycleMetrics.P50Seconds,
                        $cycleMetrics.P95Seconds,
                        $cycleMetrics.MinSeconds,
                        $cycleMetrics.MaxSeconds,
                        $cycleMetrics.RecentWindowCount,
                        $script:sqliteWriteAttempts,
                        $script:sqliteWriteRetryCount,
                        $retryRate,
                        $script:sqliteWriteLockErrors
                    ...
                    Write-CollectorLog "PERF logging skipped due to error: $($_.Exception.Message)"
                }
            }
